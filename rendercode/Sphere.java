public class Sphere extends VObject {
	...
	public Sphere(int s, int t, World world) {
		...
		shininess=0.08; // Lower gives a shinier peak spot
		dullness=(1/shininess)+100;  // Lower is duller
		...
		double delta_phi=(Math.PI)/(steps_phi-1); 
		r=1.00;
		...
		for (int i=(steps_theta*((int)(.08*steps)));i<(steps_theta*((int)(.08*steps)))+steps_theta;i++) {
			...
			if (texture!=0) {
				...
				int endy=(int)(((abs_point_phi[k])/(Math.PI))*world.texture[texture].height);
			...
			for (int t=0; t<num_faces;t++) {
				if (face_texture[t]>0) {
					...
					for (int i=0; i<3; i++) {
						...
						int y=(int)((abs_point_phi[vref]/(Math.PI))*world.texture[texture].height);
